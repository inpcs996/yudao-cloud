...
public class OaSealServiceImpl implements OaSealService {
    ...
    public Long createSeal(OaSealSaveReqVO createReqVO) {
        ...
        // 1.2 校验保管部门有效
        deptApi.validateDeptList(singleton(createReqVO.getKeeperDeptId())).checkError();
        // 1.3 校验所属部门有效，允许与保管部门不同
        deptApi.validateDeptList(singleton(createReqVO.getDeptId())).checkError();
        // 1.4 生成印章编号，并校验唯一性
    ...
    public void updateSeal(OaSealSaveReqVO updateReqVO) {
        ...
        // 1.3 校验保管部门有效
        deptApi.validateDeptList(singleton(updateReqVO.getKeeperDeptId())).checkError();
        // 1.4 校验所属部门有效，允许与保管部门不同
        deptApi.validateDeptList(singleton(updateReqVO.getDeptId())).checkError();
