...
public class OaAnnouncementServiceImpl implements OaAnnouncementService {
    ...
    public Long createAnnouncement(OaAnnouncementSaveReqVO createReqVO, Long userId) {
        ...
        List<Long> receiverUserIds = convertList(adminUserApi.getUserListBySubordinate(userId).getCheckedData(), AdminUserRespDTO::getId);
    ...
    public int forwardAnnouncement(Long id, Long userId) {
        ...
        // 1.3 查询当前用户的下属
        List<Long> subordinateUserIds = convertList(adminUserApi.getUserListBySubordinate(userId).getCheckedData(), AdminUserRespDTO::getId);
