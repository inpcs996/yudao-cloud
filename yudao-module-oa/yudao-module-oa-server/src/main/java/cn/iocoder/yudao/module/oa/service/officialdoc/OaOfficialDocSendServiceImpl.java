...
public class OaOfficialDocSendServiceImpl implements OaOfficialDocSendService {
    ...
    public void cancelOfficialDocSend(Long id, Long userId) {
        ...
        // 2. 撤销审批流程，审批事件同步单据状态
        processInstanceApi.cancelProcessInstanceByStartUser(userId, send.getProcessInstanceId(), "申请人撤销公文发文").checkError();
    }
    ...
    private void validateOfficialDocSend(OaOfficialDocSendSaveReqVO reqVO, Long id) {
        ...
        // 2. 校验发文及接收部门
        deptApi.validateDeptList(Collections.singleton(reqVO.getSendDeptId())).checkError();
        deptApi.validateDeptList(reqVO.getMainDeptIds()).checkError();
        if (CollUtil.isNotEmpty(reqVO.getCopyDeptIds())) {
            deptApi.validateDeptList(reqVO.getCopyDeptIds()).checkError();
        }
